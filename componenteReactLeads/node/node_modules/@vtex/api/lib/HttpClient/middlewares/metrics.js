        ...
        try {
            ...
            if (ctx.config.metric && metrics) {
                ...
                if (ctx.config.verbose) {
                    console.log(`VERBOSE: ${name}.${ctx.config.label}`, Object.assign({}, extensions, errorCode || errorStatus ? { errorCode, errorStatus } : null, { millis: end
                            ? utils_1.hrToMillis(end)
                            : extensions.revalidated || extensions.router || status !== 'success'
                                ? utils_1.hrToMillis(process.hrtime(start))
                                : '(from cache)', status: ctx.response && ctx.response.status, headers: ctx.response && ctx.response.headers }));
                }
            }
            else {
                if (ctx.config.verbose) {
