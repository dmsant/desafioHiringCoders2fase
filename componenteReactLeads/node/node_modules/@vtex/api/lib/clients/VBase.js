...
class VBase extends HttpClient_1.InfraClient {
    constructor(context, options) {
        ...
        this.getJSON = (bucket, path, nullIfNotFound, conflictsResolver) => {
            const headers = conflictsResolver ? { 'X-Vtex-Detect-Conflicts': true } : {};
            const inflightKey = HttpClient_1.inflightURL;
            const metric = 'vbase-get-json';
            return this.http.get(routes.File(bucket, path), { nullIfNotFound, metric, inflightKey, headers })
                .catch((error) => {
        ...
        this.getFileMetadata = (bucket, path) => {
        ...
        this.getConflicts = (bucket) => {
            return this.http.get(routes.Conflicts(bucket), { metric: 'vbase-get-conflicts' });
        };
        this.resolveConflict = (bucket, path, content) => {
            const data = [{
                    op: 'replace',
                    path,
                    value: content,
                }];
            return this.http.patch(routes.Conflicts(bucket), data, { metric: 'vbase-resolve-conflicts' });
        };
